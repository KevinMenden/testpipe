# nf-core/testpipe: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/testpipe/usage](https://nf-co.re/testpipe/usage)
...
```console
nextflow run nf-core/testpipe --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/testpipe
```
...
First, go to the [nf-core/testpipe releases page](https://github.com/nf-core/testpipe/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
